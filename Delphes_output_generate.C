...
void Delphes_output_generate()
{
   ...
   TH1D *btag_Pt = new TH1D("btag_Pt ", "btag_Pt ", 50, 0, 150);
   ...
   TH1D *top_mass = new TH1D("top_mass ", "top_mass ", 20, 30, 500);
   ...
   for (Int_t i = 0; i < nentries; i++)
   {
      ...
      for (int j = 0; j < Jet_size; j++)
      {
         ...
         //  cout<<"no. of jet_1 "<<Jet_size<<endl;
         //	TLorentzVector Jet;
         ...
         if (Jet_BTag[j] == 1)
         {
            ...
            int bj = j;
            btag_index = bj;
            //	bTag.SetPtEtaPhiM(Jet_PT[bj], Jet_Eta[bj], Jet_Phi[bj], Jet_Mass[bj]);

            // cout<<"no. of jet "<<Jet_size<<endl;
         ...
         //  bTag.SetPtEtaPhiM(Jet_PT[btag_index], Jet_Eta[btag_index], Jet_Phi[btag_index], Jet_Mass[btag_index]);
      ...
      */
      ...
      // cout << "Jet_Pt " << Jet_PT[i] << "\n";
      {
         ...
         if (i != btag_index && abs(Jet1.Eta()) > 2.5 && Jet1.Pt() > 30.0 && Jet1.E() > 300.0)
         {
            ...
            //   forwJet.SetPtEtaPhiM(Jet_PT[i], Jet_Eta[i], Jet_Phi[i], Jet_Mass[i]);
            int Bi = i;
            //  normalJets1=Jet1;
            normal_j1_index = Bi;
            //    }
         ...
         for (int k = 0; k < Jet_size; k++)
         {
            ...
            //  {
            if (k != btag_index && k != normal_j1_index && Jet2.Pt() > 25 && abs(Jet2.Eta()) < 2.5)
            {
               int Bk = k;
               // normalJets2=Jet2;
               normal_j2_index = Bk;
               //   }
               //   if(Jet2.Pt() > 25 && abs(Jet2.Eta()) < 2.5 )
               //   {
            ...
            for (int l = 0; l < Jet_size; l++)
            {
               ...
               //   {
               if (l != btag_index && l != normal_j1_index && l != normal_j2_index && Jet3.Pt() > 25 && abs(Jet3.Eta()) < 2.5)
               {
                  int Bl = l;
                  //   normalJets3=Jet3;
                  normal_j3_index = Bl;
                  //   }
                  //   if(Jet3.Pt() > 25 && abs(Jet3.Eta()) < 2.5 )
                  //   {
   ...
   TFile outputFile1("outputFile_scale_1lakh_with_cut_norm_lumi.root", "recreate");
